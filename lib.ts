const UNAMBIGOUS_ALPHA_NUMERICS = "23456789abcdefghijkmnpqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ";
...
export const randomStringBase = (length = 5, src = UNAMBIGOUS_ALPHA_NUMERICS) => {
    let s = "";
...
export const randomStringB = (length = 5, src = UNAMBIGOUS_ALPHA_NUMERICS) => 
    Array(length).fill(undefined).map(() => src.charAt(randomInt(0, src.length))).join('');
